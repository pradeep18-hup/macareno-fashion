...
/**
 * FAKE API — simulates a real backend call (network delay + Promise),
 * so the component "fetches" data the same way it would from a real API.
 * No server involved — this all happens inside the browser.
 */
...
export class ProductDetailComponent {
  ...
  private async fetchProduct(id: number) {
    ...
    const result = await fakeFetchProductById(id); // 🔗 fake API call
